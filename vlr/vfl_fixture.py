...
class FederatedLearningFixture(object):
    ...
    def fit(self, train_data, test_data, epochs=50, batch_size=-1):
        ...
        for ep in range(epochs):
            ...
            for k, _ in self.federated_learning.guest_intr_layer.host_dense_model_dict.items():
                self.federated_learning.guest_intr_layer.host_dense_model_dict[k].internal_dense_model.train()
            ...
            for i, (X, y) in enumerate(train_loader):
                ...
                for idx, party_id in enumerate(train_data["party_list"]):
                    party_X_train_batch_dict[party_id] = X[1:][idx]
            ...
            for k, _ in self.federated_learning.guest_intr_layer.host_dense_model_dict.items():
                self.federated_learning.guest_intr_layer.host_dense_model_dict[k].internal_dense_model.eval()
